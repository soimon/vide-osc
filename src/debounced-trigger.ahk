...
class Debounce {
  __New(key, outKey, releasetime := 33.3, holdtime := 33.3) {
    this.key := key
    this.outKey := outKey
    this.keydown_action := "{" outKey " down}"
    this.keyup_action := "{" outKey " up}"
  ...
  down(*) {
    if ((time() - this.timeup) > this.releasetime || this.timeup = -1) {
      ...
      if (!GetKeyState(this.outKey) || !GetKeyState(this.key))  ; key released, reset timer
        this.timedown := time()
  ...
  up(*) {
    if ((time() - this.timedown) > this.holdtime || this.timedown = -1) {
      ; cooldown has elapsed
      if (GetKeyState(this.outKey) || GetKeyState(this.key))  ; key held, reset timer
        this.timeup := time()
